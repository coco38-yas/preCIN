...
// Ajoutez cette ligne pour utiliser l'endpoint /inscription/:id
app.delete('/inscription/:id', deleteUser);
...
// Endpoint pour lancer la sauvegarde
app.post('/api/backup', async (req, res) => {
  try {
    // Chemin du dossier à sauvegarder (à adapter)
    const folderPath = req.body.folderPath || 'C:\\xampp\\mysql\\data';
    // Chemin de destination pour le fichier compressé
    const backupDir = req.body.destination || path.join(__dirname, 'C:\\Users\\cocod\\Documents\\sauvegarde');
    ...
    output.on('close', () => {
      console.log(`Backup terminé: ${archive.pointer()} octets`);
      // Vous pouvez ici ajouter la logique d'upload sur Google Drive par exemple
      return res.json({ message: 'Backup réalisé avec succès', file: backupFilename });
    });
